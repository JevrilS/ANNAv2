...
const Chatbot = () => {
   ...
   const fetchUserInfo = async () => {
      try {
        ...
        if (response.status === 200 && data.is_logged_in && data.user_data) {
          ...
          // Only trigger RIASEC_START if the test has not already started
          if (!testStarted) {
  ...
  const savedConversation = async (user, riasecCode, riasecCourses, strandCourses) => {
   ...
   const handleTermsConditionAgree = async () => {
      ...
      try {
        let accessToken = localStorage.getItem('authToken'); // Get the current access token
        ...
        if (response.status === 401 && refreshToken) {
          ...
          const refreshData = await refreshResponse.json();
          if (refreshResponse.status === 200) {
            accessToken = refreshData.access; // Update the access token
            localStorage.setItem('authToken', accessToken); // Save the new access token
            response = await checkLoginStatus(accessToken); // Retry checking login status with the new token
          } else {
            // If refreshing the token fails, clear the tokens and set login state to false
